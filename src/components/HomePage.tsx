...
const HomePage = ({ sideBarToggle, setSideBarToggle }: HomepageProps) => {
    const { currentStep, panStatus, fetchData, finish, steps, preview, selectedFile, imge, cameraType } = useStore();
    const [capturelocationData, setCapturelocationData] = useState<any | null>();
    const [stepsStatus, setStepsStatus] = useState<any>([]);
    console.log('preview => ', preview, ' selected ==> ', selectedFile, 'imge => ', imge, 'cameratype => ', cameraType);
    return (
        <>
                    ...
                    <div className="containerboxover relative">
                        <div className="sm:flex sm:justify-between">
                            ...
                            <div className="flex w-full rounded-2xl sm:ml-8 sm:bg-white relative">
                                {currentStep === 0 ? (
                                    <Welcome steps={steps} stepsStatus={stepsStatus} setStepsStatus={setStepsStatus} />
                                ) : currentStep === 1 ? (
                                    <SupersetComponent
                                        steps={steps}
